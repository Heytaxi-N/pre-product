...
5. **白图分段**:结构预筛(1 张图 + 空标题)命中的帖调 AI 判定是否服装无关的占位图,把当日流水按占位图切成一段段
6. 找到含"凯乐石女款裙裤"的锚点所在段;**再往后**逐段用 AI 判"和锚点是不是同一件商品",是就合并进去,遇到不是的段就停
7. 段内 AI 分组成产品 → 下载 → 分类 → 写飞书 → 建文件夹

只对结构预筛命中的少量帖调 AI,不全扫。不推进进度。
