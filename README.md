...
6. **排序预览(拖拽/删图)** — AI 排好后自动弹出「排序预览.html」:图片按建议顺序铺开,**拖动**任意调序、Command 单击多选、Shift 单击连选、点 **×** 删掉不要的(不存本地)、AI 认错的尺码表点「尺码表」按钮标上(视频显示为 🎬)。点「完成并生成」→ 下载 `分类确认.json`,脚本自动接住按你的顺序建文件夹。未点「完成并生成」不会自动按 AI 顺序建文件夹,可按 `Ctrl+C` 取消。`run` 自动化模式不弹;设 `SKIP_REVIEW=1` 可跳过。
7. **写飞书** — 合并后的文案写入多维表格「信息」字段,并自动填写:
   ...
   - **型号**: 暂不由脚本填写,继续使用飞书工作流自动生成。
   文案分类接口为 `https://thor.weidian.com/wditem/cateInfo.list/1.0`;分类变更后需更新本地缓存文件。分类 AI 调用失败时会回退到文案中的明确关键词匹配,没有可靠匹配则留空。飞书自动化仍会据「信息」生成「图片名」。
...
飞书字段默认名称为「信息」「分类」「成本价」「售价」,可在配置中用 `info_field`、`category_field`、`cost_field`、`sale_field` 修改。分类字段必须是飞书多选字段,成本价和售价应为数字/货币字段;脚本不会创建新的「型号」或分类选项。
