...
飞书字段默认名称为「状态」「信息」「分类」「成本价」「售价」「型号」,可在配置中用对应的 `*_field` 修改。分类字段必须是飞书多选字段,成本价和售价应为数字/货币字段;脚本不会调用微店接口创建新的型号或分类选项。

如果已有记录的「信息」已填好,且「状态」为「待编辑」、分类和型号为空,可单独补填这两列:

```bash
python3 pick_products.py fill_fields
```

命令只更新符合这三个条件的记录,不改状态和其他字段;每次写入后都会重新读取记录校验。
