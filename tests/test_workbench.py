...
class WorkbenchTests(unittest.TestCase):
    ...
    def test_html_contains_lazy_media_and_existing_confirmation_filename(self):
        ...
        self.assertIn('if(!s.end)return {lo:a,hi:a,items:list.slice(a,a+1)};', html)
        self.assertIn('id="media-hover-preview"', html)
        self.assertIn('pointer-events:auto', html)
        self.assertIn('has-actions>img', html)
        self.assertIn("currentSrc||m.thumb||m.url", html)
        self.assertIn("original.addEventListener('load'", html)
        self.assertIn('下载原图', html)
        self.assertIn('sourceFilename', html)
        self.assertIn("download=sourceFilename", html)
        self.assertIn("download.target='_blank'", html)
        self.assertIn('scheduleHideMediaPreview', html)
