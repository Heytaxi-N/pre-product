...
#media-hover-preview{display:none;position:fixed;z-index:100;pointer-events:auto;width:min(560px,calc(100vw - 16px));height:min(640px,70vh);padding:8px;box-sizing:border-box;border-radius:10px;background:#111;box-shadow:0 12px 36px rgba(0,0,0,.35);align-items:center;justify-content:center;flex-direction:column;gap:8px}#media-hover-preview img,#media-hover-preview video{display:block;max-width:100%;max-height:100%;object-fit:contain}.preview-actions{flex:none}.preview-actions a{display:inline-block;border:1px solid #fff8;border-radius:7px;padding:6px 10px;color:#fff;text-decoration:none;font-size:12px}.preview-actions a:hover{background:#fff;color:#111}.preview-actions+img{max-height:calc(100% - 38px)}.media.video{color:#fff}.media.video img{position:absolute;inset:0}.video-label{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);background:#000a;border-radius:16px;padding:5px 9px;font-size:11px;white-space:nowrap}
...
function hideMediaPreview(){cancelHideMediaPreview();const video=$('media-hover-preview').querySelector('video');if(video)video.pause();$('media-hover-preview').replaceChildren();$('media-hover-preview').style.display='none'}
...
function showMediaPreview(m,target,index){hideMediaPreview();if(!m||!m.url)return;const box=$('media-hover-preview'),media=document.createElement(m.type==='video'?'video':'img');if(m.type==='video'){media.src=m.url;media.poster=m.thumb||videoThumbFallback(m.url);media.muted=true;media.loop=true;media.autoplay=true;media.playsInline=true;box.appendChild(media)}else{const actions=document.createElement('div');actions.className='preview-actions';const download=document.createElement('a');download.href=m.url;download.target='_blank';download.rel='noopener';download.download=sourceFilename(m.url,`原图_${index+1}.jpg`);download.textContent='下载原图';download.title='下载原图';download.addEventListener('click',e=>e.stopPropagation());actions.appendChild(download);box.append(actions,media);media.alt='原图，可右键另存为'}box.style.display='flex';const r=target.getBoundingClientRect(),gap=12,w=box.offsetWidth,h=box.offsetHeight;let left=r.right+gap;if(left+w>innerWidth-8)left=r.left-w-gap;box.style.left=Math.max(8,left)+'px';box.style.top=Math.max(8,Math.min(r.top,innerHeight-h-8))+'px';if(m.type==='video')media.play().catch(()=>{})}
